...
```{r}
datos_organizados <- datos_hospital_4 |> 
  relocate(case_id, infection_date, anyo_infeccion)
```


Práctica grupal:

En grupos y usando git y un repositorio de github, divida las siguientes tareas con sus compañeras sobre preparación y limpieza de datos:
...
e. Revise los valores distintos que hay en la columna hospital. (Ooops! Al hacer esto, vemos que hay algo extraño. Al parecer, varios hospitales se escribieron de formas distintas pero parecen ser el mismo. Por ejemplo, Central Hopital y Central Hospital 
son la misma entidad. Igual con varios de los otros. Es común que en medio de nuestros análisis nos vayamos dando cuenta de que nuestros datos necesitan más 
limpieza. Use las funciones estudiadas anteriormente para homogenizar los valores), además cambie los NA por "No especifica" y "Other" por "Otros".

f. Cree una nueva columna que se llame mes_infeccion, que indique el mes de la fecha
de infección.

g. Cree una tabla que indique cuántos casos de infección se presentaron por hospital en el mes de febrero del año 2015.